    ...
    return (
      <div className="min-h-screen bg-[#0A0A0F] p-6">
        <div className="max-w-lg mx-auto rounded-2xl border border-white/10 bg-white/5 p-8 text-center">
          ...
          <Link href="/" className="inline-flex items-center gap-2 px-4 py-2 bg-cyan-600 hover:bg-cyan-500 rounded-xl text-white text-sm font-medium">
            Back to home
          </Link>
        </div>
      </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {ticket.attachmentUrls.map((url, idx) => {
                          ...
                          return (
                            <a key={idx} href={url.startsWith('http') ? url : url.startsWith('/') ? url : `/${url}`} target="_blank" rel="noopener noreferrer" className="block rounded-lg border border-amber-500/30 overflow-hidden hover:border-amber-400/50 transition-colors">
                              {isImage ? (
