...
type ReasonRow = { reasonId: string; label: string; count: number };
type ProvinceBlock = {
  province: string;
  count: number;
...
export default function ProviserMaintenancePage() {
  return (
    <ProviserPageGuard requirePrivateWorkspace>
      {({ membership }) => <MaintenanceContent membership={membership} />}
    </ProviserPageGuard>
  );
}

function MaintenanceContent({ membership }: { membership: ProviserMembership }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/provisor-private-company/maintenance-reasons/analytics?${params}`, {
      credentials: 'include',
    })
      .then((r) => r.json())
      .then((data) => {
        if (data.success) {
          setTotal(data.totalWithReason ?? 0);
  ...
  const filteredCases = useMemo(() => {
    ...
    return cases.filter((c) => {
      ...
      return (
        ...
        c.reasonLabel.toLowerCase().includes(q) ||
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="Completion reasons by province and department scope."
        actions={
          <select
            value={days}
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <SearchInput value={search} onChange={setSearch} placeholder="Search maintenance cases…" />
        <select
          value={province}
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 mb-6">
            <StatCard label="With reason" value={total} />
            <StatCard label="Reason types" value={byReason.length} />
            ...
            <Card>
              <CardBody>
                ...
                <ul className="space-y-2 text-sm max-h-72 overflow-y-auto">
                  {byReason.map((r) => (
                    <li key={r.reasonId || r.label} className="flex justify-between gap-2">
                      <span className="text-slate-300">{r.label}</span>
                      <span className="text-sky-300 font-medium tabular-nums">{r.count}</span>
                    </li>
                  ))}
                </ul>
              </CardBody>
                  ...
                  {byProvince.map((p) => (
                    <li key={p.province}>
                      <p className="font-medium text-white">
                        {p.province} <span className="text-slate-500">({p.count})</span>
                      </p>
          ...
          <h2 className="text-sm font-semibold text-slate-400 mb-3">Recent completions</h2>
          {!filteredCases.length ? (
            <EmptyState message="No maintenance cases in this period." />
          ) : (
            <ul className="space-y-2">
              {filteredCases.slice(0, 80).map((c) => (
                <li key={`${c.ticketId}-${c.completedAt}`}>
                  <Card>
                    <CardBody className="py-3">
                      <div className="flex flex-wrap justify-between gap-2">
                        <p className="font-medium text-white">{c.siteName || c.ticketId}</p>
                        <span className="text-xs text-sky-300">{c.reasonLabel}</span>
                      </div>
