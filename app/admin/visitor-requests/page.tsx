...
export default function VisitorRequestsAdminPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/visitor-requests?excludeSlug=quality-control-supervision');
      const data = await res.json();
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <h1 className="text-2xl font-bold text-gray-900">Visitor Requests (non-QC)</h1>
          {pendingCount > 0 && (
              ...
              {list.map((r) => {
                ...
                <tr key={r.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-sm">
                    {(r.serviceSlug === 'enterprise-networking' || r.serviceSlug === 'quality-control-supervision') ? (
                      <select
