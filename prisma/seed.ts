...
async function main() {
  ...
  // Digital Coordinator Platform: default company and admin user
  console.log('📋 Seeding Digital Coordinator Platform...')
  const coordinatorSalt = process.env.COORDINATOR_PASSWORD_SALT || 'usmart-cp-salt'
  const coordinatorPasswordHash = require('crypto')
    .createHash('sha256')
    .update(coordinatorSalt + 'Admin@Coordinator123')
    .digest('hex')

  const planBasic = await prisma.coordinatorSubscriptionPlan.upsert({
    where: { tier: 'BASIC' },
    update: {},
    create: { tier: 'BASIC', name: 'Basic', amountCents: 2900, interval: 'month', stripePriceId: null },
  })
  const planPro = await prisma.coordinatorSubscriptionPlan.upsert({
    where: { tier: 'PROFESSIONAL' },
    update: {},
    create: { tier: 'PROFESSIONAL', name: 'Professional', amountCents: 7900, interval: 'month', stripePriceId: null },
  })
  const planEnt = await prisma.coordinatorSubscriptionPlan.upsert({
    where: { tier: 'ENTERPRISE' },
    update: {},
    create: { tier: 'ENTERPRISE', name: 'Enterprise', amountCents: 19900, interval: 'month', stripePriceId: null },
  })

  const coordCompany = await prisma.coordinatorCompany.upsert({
    where: { slug: 'usmart-demo' },
    update: {},
    create: {
      name: 'U-Smart Demo Company',
      slug: 'usmart-demo',
  ...
  await prisma.coordinatorUser.upsert({
    where: { companyId_email: { companyId: coordCompany.id, email: 'admin@coordinator.usmart.com' } },
    update: { passwordHash: coordinatorPasswordHash },
    create: {
      email: 'admin@coordinator.usmart.com',
